...
interface ServicesCTAProps {
  cta: Dictionary["servicesPage"]["cta"];
  modal: Dictionary["modal"];
}

export function ServicesCTA({ cta, modal }: ServicesCTAProps) {
      ...
      <section className="bg-surface py-24 md:py-32">
        <div className="container">
          <div className="mx-auto max-w-2xl text-center">
            <Reveal>
              <h2 className="font-display text-4xl font-light leading-tight text-ink md:text-5xl lg:text-6xl">{cta.heading}</h2>
            </Reveal>
            <Reveal delay={150}>
              <p className="mt-6 text-base leading-relaxed text-ink-muted md:text-lg">{cta.body}</p>
            </Reveal>
            <Reveal delay={300}>
              <button
                type="button"
