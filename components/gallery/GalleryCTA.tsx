...
interface GalleryCTAProps {
  cta: Dictionary["galleryPage"]["cta"];
  modal: Dictionary["modal"];
}

export function GalleryCTA({ cta, modal }: GalleryCTAProps) {
  const [open, setOpen] = useState(false);

  return (
    <>
      <section className="border-t border-border bg-secondary py-24 text-white md:py-32">
